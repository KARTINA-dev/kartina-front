...
export class CameraHelper {
  ...
  update(time: number): void {
    ...
    if (this.isPlaying) {
      ...
      if (this.animationClip && t > this.animationClip.duration) {
        this.isPlaying = false;
    ...
    if (this.pois.length > 0) {
      ...
      if (!this.pois[0].quaternion.isQuaternion && !this.pois[0].position.isVector3) {
        for (let i = 0; i < this.pois.length; i++) {
          ...
          p.quaternion = new Quaternion(...p.quaternion);
      ...
      this.animationClip = new AnimationClip(undefined, tweenStartTime, [
  ...
  private export({ draft }: { draft: boolean }): void {
    if (this.pois.length > 0) {
  ...
  private handleEvents(event: Event): void {
    ...
    if (index) {
      if (target.classList.contains(DOMClass.visit)) {
        this.goToPoi(parseInt(index));
      } else if (target.classList.contains(DOMClass.remove)) {
        this.removePoi(parseInt(index));
      } else if (target.classList.contains(DOMClass.duration)) {
        this.updatePoi(parseInt(index), { duration: parseFloat((<HTMLInputElement>event.target).value) });
      } else if (target.classList.contains(DOMClass.ease)) {
        this.updatePoi(parseInt(index), { ease: (<HTMLSelectElement>event.target).value });
      } else if (target.classList.contains(DOMClass.moveUp)) {
        this.movePoi(parseInt(index), -1);
      } else if (target.classList.contains(DOMClass.moveDown)) {
        this.movePoi(parseInt(index), 1);
      }
      this.createClip();
    }
