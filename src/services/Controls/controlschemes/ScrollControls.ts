...
import { ScrollAdaptor, ScrollAdaptorEvent } from '../adaptors/ScrollAdaptor';
...
export class ScrollControls implements BaseControls {
  ...
  constructor(cameraRig: CameraRig, props: ScrollControlsProps) {
    ...
    this.cameraStart = props.cameraStart || this.cameraStart;
    this.cameraEnd = props.cameraEnd || this.cameraEnd;
    this.scrollActions = props.scrollActions || this.scrollActions;
    this.buffer = props.buffer || this.buffer;
    this.calculateStops();
  ...
  enable(): void {
    ...
    this.scrollAdaptor.addEventListener('update', this.onScroll as any);
    this.enabled = true;
  }

  disable(): void {
    this.scrollAdaptor.disconnect();
    this.scrollAdaptor.removeEventListener('update', this.onScroll as any);
    this.enabled = false;
  }
  ...
  private onScroll(event: ScrollAdaptorEvent): void {
    const progress = event.dampenedValues.scrollPx;

    if (progress !== null && progress >= this.cameraBufferedStartPx && progress <= this.cameraBufferedEndPx) {
      this.cameraRig.setAnimationPercentage(mapRange(progress, this.cameraStartPx, this.cameraEndPx, 0, 1));
    }
    this.scrollActions.forEach((action) => {
      if (progress !== null && progress >= action.bufferedStartPx && progress <= action.bufferedEndPx) {
