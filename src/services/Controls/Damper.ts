...
export class Damper {
  ...
  private readonly deltaValues: DamperValues = {};
  ...
  update(): void {
    ...
    for (const key in this.values) {
      deltas[key] = (this.targetValues[key] || 0) - (this.values[key] || 0);
    ...
    if (approached) {
      ...
      for (const key in this.values) {
        ...
        this.values[key] = (this.values[key] || 0) + (this.deltaValues[key] || 0);
      }
    }
  }
  ...
  addToTarget(key: string, value: number): void {
    this.targetValues[key] = (this.targetValues[key] || 0) + value;
    this.hasReached = false;
  }
   ...
   */
  resetAll(value: number | null): void {
    for (const key in this.values) {
