...
export class KeyboardAdaptor extends BaseAdaptor {
  private readonly type: KeyboardAdaptorType = 'discrete';
  private damper: Damper;
  private dampingFactor = 0.5;
  private incrementor = 1;
  private keyMapping: KeyMapping = defaultKeyMap;
  ...
  constructor(props: KeyboardAdaptorProps) {
    ...
    this.keyMapping = props.keyMapping || this.keyMapping;
    /** Only used for continuous adaptor, value between 0 and 1. Defaults to 0.5 */
    this.dampingFactor = props.dampingFactor || this.dampingFactor;
    /** Only used for continuous adaptor, the amount to increment the target value on each keydown event. Defaults to 1 */
    this.incrementor = props.incrementor || this.incrementor;
    /** Prevent event bubbling. Defaults to true */
    this.preventBubbling = props.preventBubbling || this.preventBubbling;
  ...
  disconnect(): void {
    document.removeEventListener('keyup', (event) => this.onKeyUp(event), true);
    document.removeEventListener('keydown', (event) => this.onKeyDown(event), true);
